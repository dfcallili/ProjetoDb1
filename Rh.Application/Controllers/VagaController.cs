...
namespace Rh.Application.Controllers
{
    ...
    public class VagaController : Controller
    {
        ...
        // GET api/getTecnologiaByVagaId/5
        [HttpGet("getTecnologiaByVagaId/{id}"), Produces("application/json")]
        // Matches '/vaga/getTecnologiaByVagaId/{id}'
        public IActionResult GetTecnologiaByVagaId(int id)
        {
            try
            {
                List<VagaTecnologiaDto> lista = vagaService.GetTecnologiaByVagaId(id);
                if (lista.Any())
                    return Ok(lista);
                else
                    return NotFound("Nenhuma tecnologia cadastrada.");
            }
            catch (Exception ex)
            {
                _logger.LogCritical(ex, string.Format("Houve um erro ao buscar uma Vaga ID {0}.", id));
        ...
        // POST api/calcularClassificacao
        [HttpPost("calcularClassificacao"), Produces("application/json")]
